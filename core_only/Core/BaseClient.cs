...
public abstract class BaseClient
{
    ...
    protected async Task<ApiResponse<TResponse>> CreateAsync<TRequest, TResponse>(string endpoint, TRequest data)
    {
    ...
    protected async Task<ApiResponse<TResponse>> UpdateAsync<TRequest, TResponse>(string endpoint, TRequest data)
    {
        var url = BuildUrl(endpoint);
    ...
    private string BuildUrl(string endpoint, QueryOptions? options = null)
    {
        ...
        if (options != null)
        {
            ...
            if (!string.IsNullOrEmpty(options.Order))
                queryParams.Add($"order={Uri.EscapeDataString(options.Order ?? string.Empty)}");
            
            if (!string.IsNullOrEmpty(options.Fields))
                queryParams.Add($"fields={Uri.EscapeDataString(options.Fields ?? string.Empty)}");

            if (options.Query?.Any() == true)
            {
                foreach (var kvp in options.Query)
                {
                    queryParams.Add($"{kvp.Key}={Uri.EscapeDataString(kvp.Value?.ToString() ?? string.Empty)}");
                }
            }
